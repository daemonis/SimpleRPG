class SimpleRPG
{
    ...
    public static void CollectCharacterInformation()
    {
        ...
        playerChoices[0] = ValidateAndGetInput(); // Checks if null. If null then prompts to enter another value.

        WriteText($"\nGreetings, {playerChoices[0]}. From where do you hail?");

        playerChoices[1] = ValidateAndGetInput(); // Checks if null. If null then prompts for another value.

        WriteText($"\n{playerChoices[0]} of {playerChoices[1]}. That has a nice ring to it, doesn't it? Now, are ye a lass or lad?");

        playerChoices[2] = ValidateAndGetInput(); // Checks if null. If null then prompts for another value.
